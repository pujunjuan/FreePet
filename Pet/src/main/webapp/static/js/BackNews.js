...
function build_emps_table(result) {
    ...
    $.each(emps, function (index, item) {
        ...
        var nID = $("<td></td>").append(item.nid);
        var ntitle = $("<td></td>").append(item.ntitle);
        var time=transferTime(item.ntime);
        var oTime = $("<td></td>").append(time);
        //操作下拉
        var del=$("<li></li>").addClass("del_btn").append($("<a href='#'></a>").append("删除"));
        del.attr("delID",item.nid);
        var upd=$("<li></li>").addClass("upd_btn").append($("<a href='#'></a>").attr({"data-toggle":"modal","data-target":"#tagModal"}).append("修改"));
        upd.attr("updID",item.nid);
        var detail=$("<li></li>").addClass("detail_btn").append($("<a></a>").append("查看"));
        detail.attr("detail",item.nid);
...
$(document).on("click", ".upd_btn", function () {
    ...
    $.ajax({
        url: "/Back/BackNewsByID",
        data: "id="+id,
        type: "GET",
        success: function (result) {
           var Info=result.datas.News;
            $("#nid").val(Info.nid);
           $("#edititle").val(Info.ntitle);
            $("#editname").val(Info.ncontent);
...
$(document).on("click", "#updata_btn", function () {
    $.ajax({
        url: "/Back/BackNewsUpdata",
        data: $("#tagModal form").serialize(),
        type: "PUT",
        success: function (result) {
            //关闭模态框
            $('#tagModal').modal('hide');
...
$(document).on("click", ".detail_btn", function () {
    ...
    $.ajax({
        ...
        type: "GET",
        success: function (result) {
            window.location.href="/Back/goBackNewsDetails";
        }
    });
});
...
function lookNews() {
    $.ajax({
        url: "/Back/BackNewsDetail",
        type: "GET",
        success: function (result) {
            console.log(result);
           var Info=result.datas.News;
           $("#title").html(Info.ntitle);
            $("#time").html(transferTime(Info.ntime));
            $("#con").html(Info.ncontent);
        }
    });
}
